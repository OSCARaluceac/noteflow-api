    ...
    <>
      ...
      <script dangerouslySetInnerHTML={{ __html: `
        ...
        const EPS = [
          ...
          {m:'POST',p:'/misiones',d:'Crear misión',auth:true,b:'{"title":"Nueva misión","categoria":"Caza","rango":"B"}'},
          {m:'GET',p:'/misiones/:id',d:'Misión por ID',auth:true,b:null},
          {m:'PATCH',p:'/misiones/:id',d:'Actualizar misión',auth:true,b:'{"completed":true}'},
          {m:'DELETE',p:'/misiones/:id',d:'Eliminar misión',auth:true,b:null},
        ];
        function render() {
          const list = document.getElementById('ep-list');
          let html = '', shownNotes = false, shownMisiones = false;
          EPS.forEach((ep, i) => {
            if (ep.auth && ep.p.startsWith('/notes') && !shownNotes) {
              html += '<div class="section-title" style="margin:28px 0 12px">Notas & Items — requieren JWT</div>';
              shownNotes = true;
            }
            if (ep.p.startsWith('/misiones') && !shownMisiones) {
              html += '<div class="section-title" style="margin:28px 0 12px">Misiones — requieren JWT</div>';
              shownMisiones = true;
            }
