    ...
    <html lang="es">
      <head>
        ...
        <style>{`
          *{box-sizing:border-box;margin:0;padding:0}
          ...
          .field input{background:#0a0a0b;border:1px solid #3f3f46;border-radius:6px;padding:8px 12px;color:#fafaf9;font-size:14px;outline:none;transition:border-color 0.15s}
          .field input:focus{border-color:#c5a028}
          .btn{padding:8px 20px;border-radius:6px;font-size:13px;font-weight:600;cursor:pointer;border:none;white-space:nowrap;transition:opacity 0.15s}
          .btn:hover{opacity:0.85}
          .btn-gold{background:#c5a028;color:#0a0a0b}
          .btn-outline{background:transparent;border:1px solid #3f3f46;color:#a8a29e}
          .btn-outline:hover{border-color:#c5a028;color:#c5a028}
          ...
          .ep-header{display:flex;align-items:center;gap:12px;padding:14px 16px;cursor:pointer;user-select:none;transition:background 0.15s}
          ...
          textarea,input.param{width:100%;background:#0a0a0b;border:1px solid #3f3f46;border-radius:6px;padding:10px 12px;color:#fafaf9;font-family:monospace;font-size:12px;outline:none;transition:border-color 0.15s}
          textarea{resize:vertical;min-height:80px}
          textarea:focus,input.param:focus{border-color:#c5a028}
          .try-btn{background:#c5a028;color:#0a0a0b;border:none;border-radius:6px;padding:8px 16px;font-size:12px;font-weight:700;cursor:pointer;margin-top:10px;transition:opacity 0.15s}
      ...
      <body>
        <header className="header">
          <div>
            <div className="logo-text">⚔ NOTEFLOW API</div>
            <div className="logo-sub">REST API · PostgreSQL · JWT Auth</div>
          </div>
        ...
        <div className="hero">
          ...
          <div className="auth-panel">
            ...
            <div className="auth-row">
              <div className="field"><label>Usuario</label><input id="au" placeholder="aventurero" /></div>
              <div className="field"><label>Contraseña</label><input id="ap" type="password" placeholder="••••••••" /></div>
              <button className="btn btn-gold" onClick="doLogin()">Entrar</button>
              <button className="btn btn-outline" onClick="doRegister()">Registrarse</button>
            </div>
            <div id="auth-status" className="status"></div>
          </div>
        </div>

        <div className="endpoints">
          <div className="section-title">Auth — públicos</div>
          <div id="ep-list"></div>
        </div>
        ...
        <script dangerouslySetInnerHTML={{ __html: `
          ...
          const EPS = [
            ...
            {m:'PATCH',p:'/notes/:id',d:'Actualizar campos de una nota',auth:true,b:'{"title":"Título actualizado"}'},
            ...
            {m:'POST',p:'/notes/:id/checklist-items',d:'Añadir item a una lista',auth:true,b:'{"text":"Nueva tarea"}'},
          ...
          function render() {
            ...
            let html = '';
            let shownProtected = false;
            EPS.forEach((ep, i) => {
              if (ep.auth && !shownProtected) {
                html += '<div class="section-title" style="margin:28px 0 12px">Notas & Items — requieren JWT</div>';
                shownProtected = true;
              }
              const bodyField = ep.b ? '<div class="try-section"><div class="try-label">Body JSON</div><textarea id="b'+i+'">'+JSON.stringify(JSON.parse(ep.b),null,2)+'</textarea></div>' : '';
              const paramField = ep.p.includes(':') ? '<div class="try-section"><div class="try-label">'+( ep.p.includes('itemId') ? 'itemId' : 'id' )+' (uuid)</div><input class="param" id="param'+i+'" placeholder="xxxxxxxx-xxxx-xxxx-xxxx-xxxxxxxxxxxx" /></div>' : '';
              html += '<div class="endpoint"><div class="ep-header" onclick="toggle('+i+')"><span class="method '+ep.m+'">'+ep.m+'</span><span class="ep-path">'+ep.p+'</span>'+(ep.auth?'<span class="lock">🔒</span>':'')+'<span class="ep-desc">'+ep.d+'</span></div><div class="ep-body" id="panel'+i+'">'+paramField+bodyField+'<button class="try-btn" onclick="exec('+i+')">▶ Ejecutar</button><div class="response-box" id="res'+i+'"></div></div></div>';
            });
            list.innerHTML = html;
          }
          ...
          async function exec(i) {
            const ep = EPS[i];
            let path = ep.p;
            const paramEl = document.getElementById('param'+i);
            if (paramEl && paramEl.value.trim()) path = path.replace(/:id|:itemId/, paramEl.value.trim());
            const resEl = document.getElementById('res'+i);
            resEl.style.display = 'block';
            resEl.innerHTML = '<span style="color:#78716c">Ejecutando...</span>';
            ...
            try {
              ...
              if (r.ok && path.includes('/auth/login')) {
                try { const d = JSON.parse(text); if (d.token) { TOKEN = d.token; showStatus('Autenticado — token guardado ✓', 'ok'); } } catch(e) {}
              }
              resEl.innerHTML = '<div class="res-status '+cls+'">HTTP ' + r.status + '</div>' + text;
            } catch(e) { resEl.innerHTML = '<div class="res-status res-5">Error de red</div>' + e.message; }
          }

          async function doLogin() {
            const u = document.getElementById('au').value;
            const pw = document.getElementById('ap').value;
            if (!u || !pw) { showStatus('Introduce usuario y contraseña', 'err'); return; }
            try {
              ...
              if (r.ok && d.token) { TOKEN = d.token; showStatus('Autenticado como @'+d.user.username+' ✓', 'ok'); }
